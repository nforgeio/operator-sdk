...
namespace Neon.Operator.Analyzers.Generators
{
    ...
    public class OlmGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            sb.AppendLine(KubernetesYamlHelper.YamlSerialize(csv));
            ...
            if (maintainers?.Any(maintainer => maintainer.Value.Reviewer) == true || updateGraph != null)
            {
                ...
                var outputStringCi  = KubernetesYamlHelper.YamlSerialize(ci);
            ...
            List<AttributeSyntax>     attributes)
        {
            try
            {
                ...
                foreach (var entity in ownedEntities)
                {
                    ...
                    {
                        var obj = KubernetesYamlHelper.YamlDeserialize<dynamic>(exampleYaml, stringTypeDeserialization: false);

                        jsonStrings.Add(KubernetesJsonHelper.JsonSerialize(obj));
                        continue;
                    }
                    ...
                    {
                        jsonStrings.Add(KubernetesJsonHelper.JsonSerialize(defaultEntity));
                    }
                }
