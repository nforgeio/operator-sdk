        ...
        public async Task RegisterAllFinalizersAsync(TEntity entity)
        {
            ...
            await Task.WhenAll(
                finalizerInstanceBuilder.BuildFinalizers<TEntity>(serviceProvider.CreateScope().ServiceProvider)
                    .Where(finalizer => (finalizer.GetType().GetCustomAttribute<ResourceFinalizerAttribute>()?.RegisterWithAll == true) || (finalizer.GetType().GetCustomAttribute<ResourceFinalizerAttribute>() == null))
